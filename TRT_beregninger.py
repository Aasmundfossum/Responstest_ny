...
class TRT_beregning:
    ...
    def finn_alle_tester(self):
        ...
        def funk_finn_alle_tester(df):
            ...
            for i in range(0,len(liste_over_alle_tester)):
                if len(liste_over_alle_tester[i]) > 20:
                    liste_over_tester.append(liste_over_alle_tester[i])

            return liste_over_tester
        
        self.liste_over_tester = funk_finn_alle_tester(self.df)
    ...
    def velg_test(self):
        ...
        for i in range(0,len(self.liste_over_tester)):
            ...
            if startdiff != 5.0:
                ...
                type_test = 'Kun måling av uforstyrret temperatur'
            ...
            tekst = f'Test nr. {i+1}:  ({self.liste_over_tester[i].iloc[0,0].strftime("%d.%m.%Y")}  -  {self.liste_over_tester[i].iloc[-1,0].strftime("%d.%m.%Y")}) - {type_test}'
        ...
        if 'Kun måling av uforstyrret temperatur' in valg_av_test:
            ...
            valg_test_del1 = st.selectbox('Velg tilhørende måling av uforstyrret temperatur',options = liste_over_tester_st, index=0)
        ...
        #print(self.test_del1)
        #print(self.test_del2)

        if 'Komplett' in valg_av_test or ('Kun måling av uforstyrret temperatur' in valg_av_test and 'Kun hoveddel' in valg_test_del2) or ('Kun hoveddel' in valg_av_test and 'Kun måling av uforstyrret temperatur' in valg_test_del1):
            c1,c2,c3,c4,c5 = st.columns(5)
            with c3:
                self.knapp2 = st.checkbox('Kjør! :steam_locomotive:')
        else:
            self.knapp2 = False
            st.markdown('Velg enten en komplett test eller én måling av uforstyrret temperatur og én hoveddel.')
    ...
    def plot2(self):
        ...
        fig2.update_yaxes(range=[2, self.stabil_ledn_evne+1], row=1, col=1)
    ...
    def plot6(self):
        ...
        st.markdown("---")
        self.lag_rapport_knapp = st.checkbox('Generer rapport med disse figurene 📝')
